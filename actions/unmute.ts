...
bot.command('unmute', async ctx => {
    ...
    // status
    let command_user_status = await bot.api.getChatMember(msg.chat.id, command_user.id);
    let replied_user_status = await bot.api.getChatMember(msg.chat.id, replied_user.id);
